...
function Locations(){
    ...
    useEffect(() => {
        if(state.main.id > 0){
            setState({type:'SET_LOADING', payload:true});
            dispatch(get({
                url:`api/admin/region-locations/${state.main.id}`,
                token,
                action:(response) =>{
                    console.log(response)
                    if(response.success){
    ...
    return(
        <Layout header={<Header handleSearch={handleSearch} handleClick={() => setState({type:'SET_VISIBLE_MAIN_CREATE_FORM'})}/>}>
            <Form  
                ...
                removeLocation = {(value) => setState({type:"REMOVE_LOCATION", payload:value})}
            />
            ...
            <div className="w-full h-full px-6 overflow-y-auto pb-28">
                <Table>
                    <Head>
                        <Row>
                            <>
                            ...
                            <Cell>
                                <div className="p-2 justify-center items-center">
                                    {/* <Switcher item_id={item.id} status={item?.enabled} handleStatus={handleStatus}/> */}
                                        <Switch onColor="#34D399" checked = {true} checkedIcon = {true} width={37} height={18} onChange = {()=> setState({type:"SET_ACTIVE_FORM", payload:{id:item.id, is_active:item.is_active}})} />
                                </div>
                            </Cell>
                            <Cell>
...
const Header = ({handleSearch, handleClick}) =>(
    <div className="flex flex-row justify-between items-center">
        <div className = "flex flex-row justify-center items-start"> 
            <div>
                <IconButton tooltip = "Добавить" handleClick={handleClick} icon={<IoMdAdd className="text-3xl "/>}/>
            </div>
        </div>
    </div>
);
